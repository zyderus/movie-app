...
// Transfer files
const transfer = () => {
  return src(['**/*', '**/.*', '!node_modules/**', '!public/img/unoptimized/**', '!.env']).pipe(dest('dist/'))
}
...
const nodeMon = done => {
  const stream = nodemon({
    ext: 'js json',
    script: 'index.js',
    ignore: ['node_modules/', 'public/', 'views/', 'package.json', 'package-lock.json'],
...
const html = () => {
  return src('views/**/*').pipe(browsersync.stream())
}
